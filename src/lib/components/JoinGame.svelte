...
<div class="max-w-md mx-auto bg-white border rounded-lg p-6 shadow-sm">
  <h2 class="text-2xl font-bold mb-4">Join the Divi</h2>
  
  <form on:submit|preventDefault={handleJoin} class="space-y-4">
    <div>
      <label for="playerName" class="block text-sm font-medium">Your Name</label>
      <input
        type="text"
        id="playerName"
    ...
    <button 
      type="submit" 
      class="w-full px-4 py-2 text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:bg-blue-300"
      disabled={isLoading}
    >
